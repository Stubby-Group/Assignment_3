...
mcmc_trace(new_m1) + theme_classic()
mcmc_rank_overlay(new_m1) + theme_classic()
...
loo(new_m1)
plot(loo(new_m1))
# We don't have any data points over the 0.7 Pareto k threshold, which is great, we only have 3 that are close, which is ok.
```
...
new_prior_ma <- c(
  ...
  prior(normal(0.32, 0.8), class = sd), # Using the heterogeneity prior, but making it broader, since we are analyzing heterogeneity between participants and we might expect it to be bigger, or in any case different (we are describing a different level of the structure).
  prior(normal(1, 0.5), class = sigma),
  prior(normal(2, 0.1), class = nu)
)
...
=======
>>>>>>> 92af7362c3a9ba8bb97722937a3d42c7b55e9dbc
>>>>>>> 4d8447d687f646d18be7c8d5dd5af04d1f58f31b
```
...
ggplot(post) + 
  geom_density(aes(V1), fill = "green", alpha = 0.5)  +
  geom_density(aes(V2), fill = "blue", alpha = 0.5) + 
  xlab("Beta posterior")
```
...
print(loo_compare(loo_m1, loo_m1_ma), digits = 3) #lepd: Difference in expected predictive accuracy. This test expects the model with the strict priors to have worse predicted accuracy than the model with the meta analysis priors.
