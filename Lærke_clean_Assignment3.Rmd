...
new_f <- bf(std_Pitch_IQR ~ 1 + Diagnosis + (1 | ID))
```
...
#Prior predictive check
pp_check(new_m_prior, nsamples = 100)
...
#Prior predictive check
pp_check(new_m_prior, nsamples = 10)
pp_check(new_m_prior2, nsamples = 10)
```
...
```{r Step 3.6 - Checking the posterior uodates}
...
new_prior_ma <- c(
  ...
  prior(normal(0.5, 0.5), class = sd), # Using the heterogeneity prior, but making it broader, since we are analyzing heterogeneity between participants and we might expect it to be bigger, or in any case different (we are describing a different level of the structure).
  prior(normal(1, 0.5), class = sigma),
  prior(normal(2, 0.1), class = nu)
)
...
#Prior predictive check
pp_check(new_m_ma_prior, nsamples = 100)
```
